...
import { LawyerFocDialogComponent } from './lawyer-foc-dialog/lawyer-foc-dialog.component'
import { AnswerTypeService } from 'src/app/services/answerType.service';


@Component({
  selector: 'app-all-lawyers-foc',
  templateUrl: './all-lawyers-foc.component.html',
  styleUrls: ['./all-lawyers-foc.component.scss']
})
export class AllLawyersFocComponent implements OnInit {
  LawyerID: string;
  showimage: any;
  LawyerDetail: Array<{
    activityArea: string,
...
this.answerTypeService.myGlobalValue.next('FreeFoc')
  ...
  openLawyerDetailsDialog(showLawyerDetails) {
    ...
    const dialogRef = this.dialog.open(LawyerFocDialogComponent, {
      height: '95%',
      width: '80%',
