...
@Component({
    selector: 'app-agreement-sample2',
    templateUrl: './agreement-sample2.component.html',
    styleUrls: ['./agreement-sample2.component.scss']
})
export class AgreementSample2Component implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.route.queryParams.subscribe(params => {
            ...
            // console.log("params000")
            this.masterID = params.master_id
            // console.log("params",params)
            // Temporary before disabling business-agreements component
            if (!params.showAgreement && path.includes('agreements') && !path.includes('agreements?')) {
                console.log("No params", params)
                this.router.navigate(['/agreements'], {
                    queryParams: {
                        showAgreement: 'personal',
                    }
                });
            }
            ////////////////////////////////////////////
            this.fetchDocumentList(params.showAgreement);
    ...
    openUserInputsDialog() {
        ...
        dialogRef.afterClosed().subscribe(result => {
            ...
            setTimeout(() => {
                ...
                let data = localStorage.getItem('agreementType');
                if (data == 'business') {
                    if (location.pathname.includes('klarna/success')) {
                        console.log("Calling Klarna Function")
                        if (this.order_id) {
                            this.showLoader = true;
                            this.getKlarnaPaymentResponse(localStorage.getItem('id'));
                        }
                    }
                    if (location.pathname.includes('stripe/success')) {
                        this.showLoader = true;
                        this.getStripePaymentResponse(localStorage.getItem('id'));
                    }
                    if (location.pathname.includes('swish/success')) {
                        this.showLoader = true;
                        this.getSwishPaymentResponse(localStorage.getItem('id'));
                    }
                }
    ...
    getAgreementsHeaderImage() {
        ...
        this.ContentpagesService.getContentPages(Pagename).subscribe(response => {
            ...
            this.headerImageExist = response.doc[0].agreementHeaderImgExist;
    ...
    paymentDialogSale(): void {
        ...
        console.log('THIS IS DIALOG FOR KLARNA PAYMENT ');
        ...
        const dialogRef = this.dialog.open(DialogComponent, {
          width: width,
            ...
            panelClass: 'mat-dialog-container-primary',
            data: {
                header: this.masterID === '000'?'Mötet bekräftat' : 'Betalning bekräftad',
                content: this.masterID === '000'?'Mötet är bokat, Advokat och Klienten får en möteslänk inom kort.' : 'Du får ett mejl med din beställnings- och beställningsinformation.',
                action: 'Save',
                component: 'done'
            }
       });
        dialogRef.afterClosed().subscribe(result => {
            // console.log(result)
            if(this.masterID === '000'){
                window.open('https://admin.netlex.se/schedule-list','_self')
            }
            else{
 this.router.navigate(['/schedulerapp'], {
    ...
    fetchDocumentList(filterAgreement): void {
        this.showLoader = true;
