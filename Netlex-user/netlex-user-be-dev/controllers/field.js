...
exports.fieldCheck = function (req, res) { // duplicate field check
    ...
    if (documentId === 'undefined' && fieldType === 'local') {
        Field.find({$and: [{"field.name": fieldName}, {"isGlobal": 'global'}]}).exec(function (err, field) {
            if (field.length > 0) {
                res.status(200).json('exist')
            } else {
                Field.find({$and: [{"field.name": fieldName}, {"documentTemplateId": ''}, {"isGlobal": fieldType}]}).exec(function (err, field) {
        ...
        Field.find({$and: [{"field.name": fieldName}, {"isGlobal": 'global'}]}).exec(function (err, field) {
            if (field.length > 0) {
                ...
                Field.find({$and: [{"field.name": fieldName}, {"documentTemplateId": documentId}, {"isGlobal": fieldType}]}).exec(function (err, field) {
                    field.length > 0 ? res.status(200).json('exist') : res.status(200).json({msg: 'notFound'});
                });
            }
        });
    } else if (fieldType === 'global') {
        Field.find({$and: [{"field.name": fieldName}, {"isGlobal": fieldType}]}).exec(function (err, field) {
            field.length > 0 ? res.status(200).json('exist') : res.status(200).json({msg: 'notFound'});
        });
    }
}
